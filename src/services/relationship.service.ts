...
export class RelationshipService extends Repository<RelationshipEntity> {
  ...
  public async getRelationships(userId: string, type: string): Promise<Relationship[]> {
    const query =
      type === "followers"
        ? `SELECT u.user_id, u.first_name, u.last_name, u.profile_picture FROM public.user_entity u JOIN public.relationship_entity r ON u.user_id = r.user_id WHERE r.friend_id = $1`
        : type === "following"
        ? `SELECT u.user_id, u.first_name, u.last_name, u.profile_picture FROM public.user_entity u JOIN public.relationship_entity r ON u.user_id = r.friend_id WHERE r.user_id = $1`
